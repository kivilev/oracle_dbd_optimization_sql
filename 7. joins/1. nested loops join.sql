...
---- 1. Более классический пример: 1 - unique index scan; 2 - index range scan
select /*+ index(t emp_department_ix)*/t.*, d.department_name
  from hr.departments d
  join hr.employees t on d.department_id = t.department_id
 where d.department_id = 50;

---- 2. Отличный пример применения: 1 и 2 - unique index scan
...
---- 3. 1 - unique index scan; 2 - full table scan (всего 107 строк)
select t.*, d.department_name
  from hr.departments d
  join hr.employees t on d.department_id = t.department_id
 where d.department_id = 50;
