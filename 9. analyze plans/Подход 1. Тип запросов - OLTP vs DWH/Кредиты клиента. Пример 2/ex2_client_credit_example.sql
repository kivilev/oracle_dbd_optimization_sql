...
select /*+ monitor */trunc(c.bday, 'YYYY'), count(*)
  from client c
  join client_credit cc on c.id = cc.client_id
 group by trunc(c.bday, 'YYYY');
select * from v$sqlarea t where t.sql_text like '%trunc(c.bday%' and t.sql_text like '%monitor%';
select dbms_sql_monitor.report_sql_monitor(sql_id => 'f2p38hvkhjc54', type => 'HTML') from dual;


---- Создаем индекс и смотрим как изменился план?
...
select * from v$sql_plan t where t.sql_id = '74mrq15c3qccf';
